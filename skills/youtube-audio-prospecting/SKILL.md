...
- Use the `browser` subagent (`invoke_subagent` with `TypeName: browser`) or `search_web` to prospect authentic reference sources across open multimedia databases (YouTube, Freesound, OpenAudio, NASA Open Audio).
...
### 3. Anti-Synthetic Heuristic (Authenticity First - Strict Zero-Synthesizer Mandate)
- **STRICT PROHIBITION (ZERO SYNTHESIZERS / ZERO SYNTHETIC SOUNDS):** Do not use any sound synthesizers, synthesized tones, or Web Audio oscillators (`AudioContext.createOscillator()`, `OscillatorNode`). All audio assets MUST be authentically prospected and extracted from real YouTube videos/streams using `yt-dlp` and `ffmpeg`.
- Never generate artificial synthesized sounds or procedural waveforms to fake realistic audio; always rely on real-world organic recordings sourced from YouTube.
