...
> 🔴 **PASSO 0:** Proibido gerar código sem antes consultar a matriz e invocar a skill via `view_file` como PRIMEIRA AÇÃO DO TURNO.


## 1. UNIVERSAL SKILL INVOCATION MATRIX
| Intenção / Gatilhos Globais | Skill Obrigatória | Ação Requerida |
|---|---|---|
...
| Pesquisa de APIs / Ferramentas / MCP | `skills/technological-prospecting/SKILL.md` | Mapear contratos, protocolos e sandboxes |
| Referências web / Benchmarks de Mercado | `skills/competitive-reference-benchmarking/SKILL.md` | Buscar referências do estado-da-arte |
| Modelos 3D / CAD / Malhas Espaciais | `skills/sketchfab-prospecting-protocol/SKILL.md` | Prospecção de ativos espaciais e 3D |
| Áudio / Telemetria Acústica / SFX | `skills/youtube-audio-prospecting/SKILL.md` | Prospecção e síntese de inteligência acústica |
...
| Triagem de teses / Falsificação Causal | `skills/thesis-triage-funnel/SKILL.md` | Funil causal de validação de hipóteses |
| Construção / Refatoração de Código | `skills/master-refactoring-pipeline/SKILL.md` | Pipeline 8 fases com Zero-Hardcoding |
| Design Visual / UI / Web / CSS / PBR | `skills/visual-synthesis-engine/SKILL.md` | Design atômico, design tokens e profundidade |
| Geração de Vídeos / Gemini Omni & Veo | `skills/gemini-omni-video-generation/SKILL.md` | Síntese cinematográfica de vídeo, ótica e looping |
| Interação de Scroll / Scrollytelling / Vídeo Scrubbing / Parallax / Ícones 3D | `skills/interactive-kinetic-media-engine/SKILL.md` | Sincronização de vídeo por scroll, scrollytelling e física tátil |
| Experiência Web Ultra-Moderna / Vídeos / UI Sênior / Editorial | `skills/omni-experience-synthesis/SKILL.md` | Síntese multimodal Awwwards-tier, Veo e Glassmorphism |
| Computação Espacial / Shaders GLSL / Física | `skills/omni-multimodal-spatial-engine/SKILL.md` | Shaders procedurais, projeção 3D e matemática |
...
1. **DETERMINISTIC 6-STAGE AGI/ASI PIPELINE (ZERO-OMISSION LAW):** Para qualquer missão, criação de sistema, web app, backend ou jogo, o agente é expressamente proibido de executar apenas 1 ou 2 passos e parar. Ele DEVE OBRIGATORIAMENTE executar a esteira de 6 fases em sequência:
   ...
   - **Fase 5 (Playtesting Full HD):** `puppeteer-browser-automation` / `/browser` (Capturas Full HD 1920x1080 + `view_file` nos PNGs).
   - **Fase 6 (Tribunal Adversarial & Ponto W):** `adversarial-tribunal` + `point-w-evolutionary-engine` (Subagente Red Team caça 3 defeitos; mínimo 2 ciclos de mutação até $Q \ge 9.0$).
...
3. **EVIDENCE-GATE (Zero-Trust):** Proibido concluir sem prova empírica: 1) Compilação/execução sem erros (`exit 0`). 2) Para interfaces visuais: Capturas obrigatórias em **resolução Full HD (1920x1080)** em tela cheia via Puppeteer MCP (`puppeteer_screenshot(width: 1920, height: 1080)`) + `view_file` nos PNGs + $N \ge 2$ iterações consertando críticas. 3) Para sistemas de backend: Testes de estresse automatizados.
4. **ZERO-LAZY MANDATE:** Proibido `// TODO`, `pass`, ou código em blocos omitidos. Edite 100% do arquivo com precisão de produção. O caminho mais rigoroso é a única opção.
5. **ATIVOS & DADOS REAIS (Anti-Sintético / Anti-Mock):**
   ...
   - **Áudio & Acústica:** Proibido o uso de sintetizadores, osciladores (`OscillatorNode`) ou sons sintetizados artificiais. É MANDATÓRIO prospectar e extrair áudios reais de fontes autênticas do YouTube via prospecção acústica (`youtube-audio-prospecting`) processados via `yt-dlp`/`ffmpeg` em formatos web (.ogg, .mp3, .wav).
